from PySparse.Dcs_add import Dcs_add
from PySparse.Dcs_util import Dcs_util
import numpy as np

class Dcs_amd:
    @classmethod
    def cs_wclear(mark, lemax, w, w_offset, n):
    ...
    def cs_amd(order, A):
        ...
        # boolean ok;
        # /* --- Construct matrix C ----------------------------------------------- */
        if (not Dcs_util.CS_CSC(A)) or order <= 0 or order > 3:
            return None;
        ...
        m = A.m
        n = A.n
        dense = np.max(16, 10 * np.sqrt(n)) #/* find dense threshold */
        dense = np.min(n - 2, dense)
        if (order == 1 and n == m):
            ...
            C =  Dcs_multiply.cs_multiply(AT, A2) if A2 is not None else None #/* C=A'*A with no dense rows */
            A2 = None
        else:
        ...
        if (C is None):
            return None
        Dcs_fkeep.cs_fkeep(C, new Cs_diag(), None) #/* drop diagonal entries */
        Cp = C.p
        cnz = Cp[n]
        P = np.zeros(n + 1)      # /* allocate result */
        W = np.zeros(8 * (n + 1)) # /* get workspace */
